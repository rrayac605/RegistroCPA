	...
	public String loginSession(Model model, HttpServletResponse response, HttpServletRequest request) {
		...
		while(elementos.hasMoreElements()) {
			...
			if(element.contains("SSO")) {
				...
				usuarioOpenAM.setUmf(quitaCorchetesString(request.getAttribute("SSO_UMF").toString()));
				usuarioOpenAM.setIdPersona(quitaCorchetesString(request.getAttribute("SSO_IDPERSONA").toString()));
			}
		}
		...
		model.addAttribute("usuario", usuarioOpenAM);
